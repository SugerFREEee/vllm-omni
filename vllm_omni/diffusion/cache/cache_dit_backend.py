...
def enable_cache_for_cosyvoice3(pipeline: Any, cache_config: Any) -> Callable[[int], None]:
    """Enable cache-dit for CosyVoice3 DiT architecture.

    CosyVoice3 uses a single transformer (pipeline.transformer) with blocks
    that need to be enabled using BlockAdapter.

    Args:
        pipeline: The CosyVoice3 pipeline instance.
        cache_config: DiffusionCacheConfig instance with cache configuration.
    ...
    logger.info(
        f"Enabling cache-dit on CosyVoice3 transformer with BlockAdapter: "
        f"Fn={db_cache_config.Fn_compute_blocks}, "
        f"Bn={db_cache_config.Bn_compute_blocks}, "
        f"W={db_cache_config.max_warmup_steps}, "
    )
...
CUSTOM_DIT_ENABLERS.update(
    {
        ...
        "CosyVoice3Pipeline": enable_cache_for_cosyvoice3,
    }
)
